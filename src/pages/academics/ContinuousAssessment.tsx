...
import { subjects, students, homeworks } from '@/lib/dummy-data';
import { BookCheck, FileText } from 'lucide-react';

export default function ContinuousAssessment() {
  const student = students[0]; // Demo: Henry Murinda

  const subjectAssessments = subjects.filter(s => s.classes.includes(student.className)).map(sub => {
    const subHomeworks = homeworks.filter(h => h.subjectId === sub.id && h.className === student.className);
    const hwMarks = subHomeworks.map((h, i) => ({ name: `HW${i + 1}: ${h.title}`, mark: Math.floor(Math.random() * h.totalMarks * 0.4 + h.totalMarks * 0.5), total: h.totalMarks }));
    const inclassMarks = [{ name: 'Test 1', mark: Math.floor(Math.random() * 30 + 50), total: 100 }, { name: 'Test 2', mark: Math.floor(Math.random() * 30 + 55), total: 100 }];
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="font-display text-2xl font-bold text-foreground">Continuous Assessment Report</h1>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border bg-muted">
                ...
                <th className="text-left px-3 py-2 font-medium text-muted-foreground">In-Class Tests</th>
                ...
                <th className="text-right px-3 py-2 font-medium text-muted-foreground">Final CA Mark (%)</th>
              </tr>
            </thead>
            <tbody>
              {subjectAssessments.map(sa => (
                <tr key={sa.subject.id} className="border-b border-border hover:bg-muted/50">
                  ...
                  <td className="px-3 py-3">
                    <span className={`px-1.5 py-0.5 rounded text-xs ${sa.subject.type === 'Theory' ? 'bg-info/10 text-info' : 'bg-warning/10 text-warning'}`}>{sa.subject.type}</span>
                  </td>
                  ...
                  <td className="px-3 py-3 text-right text-foreground">{sa.subject.caPercent}%</td>
